...
SS_model = GWModelBuilder(name="GW_link_Integrated",
                          ...
                          data_folder="C:/UserData/takuyai/ownCloud/CampaspeModel/testbox/integrated/model_build/input_data",
                          model_data_folder="C:/UserData/takuyai/ownCloud/CampaspeModel/testbox/integrated/data",
                          out_data_folder="C:/UserData/takuyai/ownCloud/CampaspeModel/testbox/integrated/model_build/data_build",
...
external_data_folder = "C:/development/campaspe/GW_data/Campaspe_data"
...
if os.path.exists(SS_model.out_data_folder + rain_info_file + '.h5'):
    long_term_historic_rainfall = SS_model.load_dataframe(
        SS_model.out_data_folder + rain_info_file + '.h5')
else:
    long_term_historic_rainfall = processWeatherStations.processWeatherStations(
        weather_stations, path=os.path.join(external_data_folder, "Climate\\"))
    SS_model.save_dataframe(SS_model.out_data_folder + rain_info_file, long_term_historic_rainfall)

rain_gauges = SS_model.read_points_data(os.path.join(
    external_data_folder, "Climate\Rain_gauges.shp"))

# $%$%$%$%$%$%$%$%$%$%$%$%$%$%$%$%$%$%$%$%
# INCLUDE NSW bores in this next part too for better head representation
# at the border, i.e. Murray River
...
# getBoreDepth ... assuming that midpoint of screen interval is
# representative location and assign to layer accordingly
...
bores_shpfile = SS_model.read_points_data(
    "C:/development/campaspe/GW_data/Campaspe_data/ngis_shp_VIC/ngis_shp_VIC/NGIS_Bores.shp")

bores_filtered_from_shpfile = SS_model.points_shapefile_obj2dataframe(
    bores_shpfile, feature_id="HydroCode")
...
# final_bores.plot(kind='scatter', x="Easting", y="Northing", c="mean
# level", cmap="Spectral") # , edgecolor='None'
...
pumping_data = get_GW_licence_info.get_GW_licence_info(
    filename, path=path, out_file=out_file, out_path=out_path)
pumps_points = SS_model.read_points_data(os.path.join(
    external_data_folder, r"GW\Bore data\pumping wells.shp"))

print "************************************************************************"
print " Executing custom script: readHydrogeologicalProperties "

file_location = os.path.join(
    external_data_folder, r"GW\Aquifer properties\Hydrogeologic_variables.xlsx")
...
if os.path.exists(SS_model.out_data_folder + river_flow_file + '.h5'):
    ...
    river_flow_data = processRiverStations.getFlow(path=os.path.join(
        external_data_folder, r"SW\All_streamflow_Campaspe_catchment\\"))
...
if os.path.exists(SS_model.out_data_folder + river_stage_file + '.h5'):
    ...
    river_stage_data = processRiverStations.getStage(path=os.path.join(
        external_data_folder, r"SW\All_streamflow_Campaspe_catchment\\"))
...
river_gauges = SS_model.read_points_data(os.path.join(
    external_data_folder, r"SW\All_streamflow_Campaspe_catchment\processed_river_sites_stage.shp"))

print "************************************************************************"
print "Load in the river shapefiles"

Campaspe_river_poly = SS_model.read_polyline(
    "Campaspe_Riv.shp", path=r"C:\Workspace\part0075\MDB modelling\Campaspe_model\GIS\GIS_preprocessed\Surface_Water\Streams\\")
Murray_river_poly = SS_model.read_polyline(
    "River_Murray.shp", path=r"C:\Workspace\part0075\MDB modelling\Campaspe_model\GIS\GIS_preprocessed\Surface_Water\Streams\\")

print "************************************************************************"
print "Load in the shapefiles defining groundwater boundaries"

WGWbound_poly = SS_model.read_polyline(
    "western_head.shp", path=r"C:\Workspace\part0075\MDB modelling\testbox\input_data\\")
EGWbound_poly = SS_model.read_polyline(
    "eastern_head.shp", path=r"C:\Workspace\part0075\MDB modelling\testbox\input_data\\")
...
SS_model.model_time.set_temporal_components(
    steady_state=False, start_time=start, end_time=end, time_step='A')

# Define the grid width and grid height for the model mesh which is stored
# as a multipolygon shapefile GDAL object
print "************************************************************************"
...
hu_raster_files = ["qa_1t", "qa_2b", "utb_1t", "utb_2b", "utqa_1t", "utqa_2b",
                   "utam_1t", "utam_2b", "utaf_1t", "utaf_2b", "lta_1t", "lta_2b", "bse_1t", "bse_2b.tif"]
...
SS_model.build_3D_mesh_from_rasters(
    model_grid_raster_files, SS_model.out_data_folder_grid, 1.0, 1000.0)
...
for unit in HGU:
    SS_model.parameters.create_model_parameter(
        'Kh_' + unit, value=HGU_props['Kh mean'][HGU_map[unit]])
    SS_model.parameters.parameter_options('Kh_' + unit,
                                          ...
                                          OFFSET=0)
    SS_model.parameters.create_model_parameter(
        'Kv_' + unit, value=HGU_props['Kz mean'][HGU_map[unit]])
    SS_model.parameters.parameter_options('Kv_' + unit,
                                          ...
                                          OFFSET=0)
    SS_model.parameters.create_model_parameter(
        'Sy_' + unit, value=HGU_props['Sy mean'][HGU_map[unit]])
    SS_model.parameters.parameter_options('Sy_' + unit,
                                          ...
                                          OFFSET=0)
    SS_model.parameters.create_model_parameter(
        'SS_' + unit, value=HGU_props['SS mean'][HGU_map[unit]])
...
interp_rain = SS_model.interpolate_points2mesh(
    rain_gauges, long_term_historic_rainfall, feature_id='Name')
...
for i in [1, 2, 3, 7]:
    ...
    interp_rain[SS_model.model_mesh3D[1][0] == i] = interp_rain[SS_model.model_mesh3D[
        1][0] == i] * SS_model.parameters.param['rch_red_' + zone_map[i]]['PARVAL1']
...
# [[final_bores.loc[x, "Easting"], final_bores.loc[x, "Northing"], final_bores.loc[x, "depth"]] for x in final_bores.index]
...
bores_obs_time_series = bores_obs_time_series.rename(
    columns={'HydroCode': 'name', 'mean level': 'value'})

SS_model.observations.set_as_observations(
    'head', bores_obs_time_series, bore_points3D, domain='porous', obs_type='head', units='mAHD')


bores_in_layers = SS_model.map_points_to_raster_layers(
...
for i in range(len(hu_raster_files_reproj) / 2):
    ...
    if bores_layer.shape[0] < 4:
        ...
        bores_head_layer = np.array(final_bores["mean level"].tolist())[
            np.array(bores_in_layers[i])]
        unique_bores = np.unique(bores_layer)

        b = np.ascontiguousarray(bores_layer).view(
            np.dtype((np.void, bores_layer.dtype.itemsize * bores_layer.shape[1])))
        _, idx = np.unique(b, return_index=True)

        unique_bores = bores_layer[idx]

        interp_heads[hu_raster_files[2 * i]] = SS_model.interpolate_points2mesh(
            bores_layer, bores_head_layer, use='griddata', method='linear')
...
SS_model.initial_conditions.set_as_initial_condition(
    "Head", initial_heads_SS)  # interp_heads[hu_raster_files[0]])
...
for C14wells in SS_model.points_mapped['C14_clipped.shp']:
    ...
    for well in C14wells[1]:
        try:
            well_depth = df_C14.loc[df_C14[df_C14['Bore_id'] == int(well)].index.tolist()[
                0], 'avg_screen(m)']
...
C14_bore_points3D.rename(columns={'zone55_easting': 'Easting',
                                  'zone55_northing': 'Northing'}, inplace=True)

SS_model.observations.set_as_observations(
    'C14', C14_obs_time_series, C14_bore_points3D, domain='porous', obs_type='concentration', units='pMC')
...
pump_date_index = pd.date_range(start=datetime.datetime(
    2014, 07, 01), end=datetime.datetime(2015, 06, 30), freq='AS-JUL')
...
for pump_cell in SS_model.points_mapped['pumping wells_clipped.shp']:
    ...
    for pump in pump_cell[1]:
        ...
        if pumping_data.loc[pump, 'Top screen depth (m)'] == 0.:
            ...
            continue
        pump_depth = SS_model.model_mesh3D[0][0][row][col] - \
            pumping_data.loc[pump, 'Top screen depth (m)']
        active = False
        ...
        pump_date_index2 = pd.date_range(
            start=pump_install, end=datetime.datetime(2004, 06, 30), freq='AS-JUL')
        ...
        date_index = pd.date_range(start=SS_model.model_time.t['start_time'], end=SS_model.model_time.t[
                                   'end_time'], freq=SS_model.model_time.t['time_step'])
        pumping_data_ts = pumping_data_ts.reindex(date_index)
        pumping_data_ts = pumping_data_ts.ix[SS_model.model_time.t[
            'start_time']:SS_model.model_time.t['end_time']]
        pumping_data_ts = pumping_data_ts.fillna(0.0)

        # Now fill in the well dictionary with the values of pumping at relevant
        # stress periods where Q is not 0.0
...
# Added in 406203 as it represents stage just downstream of the weir, NB
# not currently included in list of gauges online
...
for index, riv in enumerate(new_riv):
    ...
    if riv[0] in filter_gauge_loc:
        ...
        stages[index] = river_stage_data["Mean stage (m)"].loc[river_stage_data[
            "Site ID"] == filter_gauges[gauge_ind[0]][1][0]]
...
for index, riv_cell in enumerate(SS_model.polyline_mapped['Campaspe_Riv_model.shp']):
    ...
    # SS_model.model_mesh3D[0][0][row][col] - SS_model.parameters.param['bed_depress']['PARVAL1']
    bed = beds[index]
...
for MurrayGHB_cell in SS_model.polyline_mapped['River_Murray_model.shp']:
    ...
    for lay in range(SS_model.model_mesh3D[1].shape[0]):
        ...
        MurrayGHBstage = SS_model.model_mesh3D[0][0][row][
            col] + SS_model.parameters.param['MGHB_stage']['PARVAL1']

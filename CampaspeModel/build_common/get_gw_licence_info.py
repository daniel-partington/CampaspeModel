...
def get_gw_licence_info(filename, path=None, out_file=None, out_path=None):
    ...
    if path == None:
        ...
        filename = p_j(path, filename)

    df = pandas.read_excel(filename, skiprows=10)

    cols = ['RECORD ID', 'Water System Source',
            'Trading Zone', 'Use of Water', 'Entitlement Volume']
    ...
    # print df.groupby('Trading Zone').loc['Bamawn - Lower Campaspe Valley']
    # #.sum() #.isin(trade_zone)]
    ...
    if out_path == None:
        ...
        out_file = p_j(out_path, out_file)

...
if os.path.exists(SS_model.out_data_folder + rain_info_file + '.h5'):
    ...
    long_term_historic_rainfall = processWeatherStations.processWeatherStations(
        weather_stations, path=r"C:\Workspace\part0075\MDB modelling\Campaspe_data\Climate\\")
    SS_model.save_dataframe(SS_model.out_data_folder + rain_info_file, long_term_historic_rainfall)

rain_gauges = SS_model.read_points_data(
    r"C:\Workspace\part0075\MDB modelling\Campaspe_data\Climate\Rain_gauges.shp")
...
if os.path.exists(SS_model.out_data_folder + river_flow_file + '.h5'):
    ...
    river_flow_data = processRiverStations.getFlow(
        path=r"C:\Workspace\part0075\MDB modelling\Campaspe_data\SW\All_streamflow_Campaspe_catchment\\")
...
if os.path.exists(SS_model.out_data_folder + river_stage_file + '.h5'):
    ...
    river_stage_data = processRiverStations.getStage(
        path=r"C:\Workspace\part0075\MDB modelling\Campaspe_data\SW\All_streamflow_Campaspe_catchment\\")
...
river_gauges = SS_model.read_points_data(
    r"C:\Workspace\part0075\MDB modelling\Campaspe_data\SW\All_streamflow_Campaspe_catchment\processed_river_sites_stage.shp")
...
interp_rain = SS_model.interpolate_points2mesh(
    rain_gauges, long_term_historic_rainfall, feature_id='Name', method='linear')
...
for i in [1, 2, 3, 7]:
    ...
    interp_rain[SS_model.model_mesh3D[1][0] == i] = interp_rain[SS_model.model_mesh3D[
        1][0] == i] * SS_model.parameters.param['SSrch_' + zone_map[i]]['PARVAL1']
...
#
# SS_model.initial_conditions.set_as_initial_condition("Head",
# initial_heads_SS)#interp_heads[hu_raster_files[0]])
...
for index, riv in enumerate(new_riv):
    ...
    if riv[0] in filter_gauge_loc:
        ...
        stages[index] = river_stage_data["Mean stage (m)"].loc[river_stage_data[
            "Site Name"] == filter_gauges[gauge_ind[0]][1][0]]

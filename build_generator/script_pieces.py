...
def s_basic_model_params(epsg_code):
    return """
# Define basic model parameters:
Proj_CS = osr.SpatialReference()

# {epsg_code} is the spatial reference code for the model
...
def s_gen_modelbuilder(prj_name, model_type, mesh_type, study_area):
    return """
study_data = "{study_area}_data"
...
climate_path = p_j(get_conf_set(['model_build', study_data]), 'Climate')

temp_data_path = get_conf_set(['model_build', 'temp_data'])
river_path = p_j(temp_data_path, "input_data", "Waterways")
sw_data_path = p_j(temp_data_path, study_data+"/SW/All_streamflow_Campaspe_catchment")
...
model_params = {{
    ...
    study_data: get_conf_set(['model_build', study_data]),
...
""".format(prj=prj_name, t_model=model_type, t_mesh=mesh_type, study_area=study_area)
...
rain_gauges = SS_model.read_points_data(p_j(climate_path, "{gauges_shp}"))
""".format(stn_list=station_list, nfo_file=rain_info_file, gauges_shp=rain_gauges_shp)


def s_bore_data(gw_licence_data):
    return """
...
if os.path.exists(bore_lf) & os.path.exists(bore_if):
    ...
    bore_data_levels, bore_data_info = getBoreData.getBoreData(base_path=getattr(SS_model, study_data))
...
ngis_bore_shp = p_j(
    temp_data_path, study_data, "ngis_shp_VIC", "ngis_shp_VIC", "NGIS_Bores.shp")
...
filename = "{gw_licence_data}"
path = out_path = p_j(temp_data_path, study_data, "GW", "Bore data")
...
""".format(gw_licence_data=gw_licence_data)


def s_hydrogeo_properties(hydrogeo_var_data):
    return """
if VERBOSE:
    print "************************************************************************"
...
file_location = p_j(
    temp_data_path, "Campaspe_data/GW/Aquifer properties/{hydrogeo_var_data}")
HGU_props = readHydrogeologicalProperties.getHGUproperties(file_location)
""".format(hydrogeo_var_data=hydrogeo_var_data)


def s_include_c14(c14_data):
    return """
if VERBOSE:
    print "************************************************************************"
    print "Get the C14 data"

C14_points = SS_model.read_points_data(temp_data_loc + study_data + r"\Chemistry\C14.shp")

C14data = temp_data_loc + study_data + r"\Chemistry\{c14_data}"
df_C14 = pd.read_excel(C14data)
df_C14.drop_duplicates(subset=["Bore_id"], inplace=True)
df_C14.dropna(inplace=True)
""".format(c14_data=c14_data)


def gen_polyline(shps, suffix, in_path_var):
    polys = ""
    for shp in shps:
        polys += '{shp}{suffix}_poly = SS_model.read_polyline("{fn}", path={in_path_var})\n'\
            .format(shp=shp, suffix=suffix, fn=shps[shp], in_path_var=in_path_var)
    # End for
...
def s_process_river_stations(river_shps):
    """
    :param river_shps: list, shapefiles for target rivers
    """

    polys = gen_polyline(river_shps, "_river", "river_path")
...
def s_load_gw_boundary_shp(gw_bound_shps):

    polys = gen_polyline(gw_bound_shps, "", "model_build_input_path")

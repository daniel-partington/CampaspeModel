...
SS_model = GWModelBuilder(name="01_steady_state_modern", 
                          data_folder=r"C:\Workspace\part0075\MDB modelling\testbox\input_data\\",
                          out_data_folder=r"C:\Workspace\part0075\MDB modelling\testbox\01_steady_state_modern\\",
...
bores_filtered_shpfile = SS_model.read_points_data(os.path.join(SS_model.out_data_folder, "NGIS_Bores_clipped.shp"))
...
# NOTE: Basement is set to be max of 20m thick and min of 10m thick.
...
for i in [1,2,3,7]:
    if i != 7:
        ...
        SS_model.parameters.create_model_parameter('rch_red_'+zone_map[i], value=0.0)
        SS_model.parameters.parameter_options('rch_red_'+zone_map[i], 
                                              PARTRANS='fixed', 
                                              PARCHGLIM='factor', 
                                              PARLBND=0., 
...
#
initial_heads_SS = np.full(SS_model.model_mesh3D[1].shape, 150.)
...
for pump_cell in SS_model.points_mapped['pumping wells_clipped.shp']:
    ...
    for pump in pump_cell[1]: 
        ...
        pumping_data_ts = pumping_data_ts.resample('M', how='mean')
        ...
        date_index = pd.date_range(start=start, end=end, freq='M')
        pumping_data_ts = pumping_data_ts.reindex(date_index)    
        pumping_data_ts = pumping_data_ts.ix[start:end]
        pumping_data_ts = pumping_data_ts.fillna(0.0)

        pumping_mean = pumping_data_ts.mean()
        ...
        try:
            wel[0] += [[active_layer, row, col, pumping_mean]]
        except:
            wel[0] = [[active_layer, row, col, pumping_mean]]
...
drain_poly = SS_model.read_polyline("Drain_Clip.shp", path=r"C:\Workspace\part0075\MDB modelling\testbox\input_data\SW\\") 
SS_model.map_polyline_to_grid(drain_poly)

SS_model.parameters.create_model_parameter('drain_drop', value=0.01)
SS_model.parameters.parameter_options('drain_drop', 
                                      PARTRANS='log', 
                                      PARCHGLIM='factor', 
                                      PARLBND=0.001, 
                                      PARUBND=0.1, 
                                      ...
                                      SCALE=1, 
                                      OFFSET=0)
SS_model.parameters.create_model_parameter('Kv_drain', value=5E-3)
SS_model.parameters.parameter_options('Kv_drain', 
                                      PARTRANS='log', 
                                      PARCHGLIM='factor', 
                                      PARLBND=1E-8, 
                                      PARUBND=20, 
...
for drain_cell in SS_model.polyline_mapped['Drain_Clip_model.shp']:
    row = drain_cell[0][0]
    col = drain_cell[0][1]
    if SS_model.model_mesh3D[1][0][row][col] == -1:
        continue
    #print SS_model.model_mesh3D
    drain_bed = SS_model.model_mesh3D[0][0][row][col] - SS_model.parameters.param['drain_drop']['PARVAL1']
    drain_cond = drain_cell[1] * drain_width_avg * SS_model.parameters.param['Kv_drain']['PARVAL1'] / drain_bed_thickness
...
SS_model.boundaries.create_model_boundary_condition('Drain', 'drain', bc_static=True)
SS_model.boundaries.assign_boundary_array('Drain', drain)


print "************************************************************************"
print " Mapping Channels to grid"

channel_poly = SS_model.read_polyline("Channel_Clip.shp", path=r"C:\Workspace\part0075\MDB modelling\testbox\input_data\SW\\") 
SS_model.map_polyline_to_grid(channel_poly)

SS_model.parameters.create_model_parameter('chan_drop', value=0.01)
SS_model.parameters.parameter_options('chan_drop', 
                                      PARTRANS='log', 
                                      PARCHGLIM='factor', 
                                      PARLBND=0.001, 
                                      PARUBND=0.1, 
                                      ...
                                      SCALE=1, 
                                      OFFSET=0)
SS_model.parameters.create_model_parameter('Kv_chan', value=5E-3)
SS_model.parameters.parameter_options('Kv_chan', 
                                      PARTRANS='log', 
                                      PARCHGLIM='factor', 
                                      PARLBND=1E-8, 
                                      PARUBND=20, 
...
for channel_cell in SS_model.polyline_mapped['Channel_Clip_model.shp']:
    ...
    if (row, col) in camp_river_locs:
        continue
    #print SS_model.model_mesh3D
    channel_stage = SS_model.model_mesh3D[0][0][row][col]
    channel_bed = SS_model.model_mesh3D[0][0][row][col] - SS_model.parameters.param['chan_drop']['PARVAL1']
    channel_cond = channel_cell[1] * channel_width_avg * SS_model.parameters.param['Kv_chan']['PARVAL1'] / channel_bed_thickness
...
SS_model.boundaries.create_model_boundary_condition('Channel', 'channel', bc_static=True)
SS_model.boundaries.assign_boundary_array('Channel', channel)
